...
export class HeroeService {
  ...
  searchHeroes(term: string): Observable<Heroe[]> {
    ...
    return this.http.get<Heroe[]>(`api/heroes/?name=${term}`).pipe(
  ...
  addHero (hero: Heroe): Observable<Heroe> {
    return this.http.post<Heroe>(this.heroesUrl, hero, httpOptions).pipe(
      tap((hero: Heroe) => this.log(`added hero w/ id=${hero.id}`)),
  ...
  deleteHero (hero: Heroe | number): Observable<Heroe> {
    ...
    return this.http.delete<Heroe>(url, httpOptions).pipe(
  ...
  private log(message: string) {
    this.messageServ.add('HeroService: ' + message);
  }
